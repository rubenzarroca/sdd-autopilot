...
### Opus Routing for plan-architect (complexity high/critical)
...
### Headless Mode + MCP Connection Fix

#### Critical Fix
- **plugin.json**: use `${CLAUDE_PLUGIN_ROOT}` for MCP server path — fixes server not connecting when cwd != plugin dir
- **plugin.json / engine/package.json**: bumped to `2.0.0` so the marketplace installer detects the path change and re-syncs cached installs (the `build/` → `dist/` migration was previously invisible to the installer because the version stayed at `1.1.0`)
